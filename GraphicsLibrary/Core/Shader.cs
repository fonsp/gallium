	...
	public class Shader
	{
        ...
        public static Shader particleShader
		{
			get
			{
				return new Shader
				{
					vertexShader = File.ReadAllText(@"Content/shaders/particle.vsh"),
					fragmentShader = File.ReadAllText(@"Content/shaders/particle.fsh")
				};
			}
		}

		private static Shader particleShaderCompiledi;
		public static Shader particleShaderCompiled
		{
			get
			{
				if(particleShaderCompiledi == null)
				{
					particleShaderCompiledi = particleShader;
				}
				if(particleShaderCompiledi.Compiled == false)
				{
					particleShaderCompiledi.GenerateShaders();
				}
				return particleShaderCompiledi;
			}
		}
